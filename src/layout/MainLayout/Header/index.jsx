...
const GitHubStarButton = ({ starCount, isDark }) => {
    ...
    return (
        <Link href='https://github.com/VictorBondaruk/BookGenerator' target='_blank' underline='none' sx={{ display: 'inline-flex' }}>
            <Box
                sx={{
                    display: 'flex',
                    alignItems: 'center',
    ...
    useEffect(() => {
        if (isCloud || isOpenSource) {
            const fetchStarCount = async () => {
                try {
                    const response = await fetch('https://api.github.com/repos/VictorBondaruk/BookGenerator')
