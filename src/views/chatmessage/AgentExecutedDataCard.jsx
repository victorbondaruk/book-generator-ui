...
import { BOOK_GENERATOR_CREDENTIAL_ID, AGENTFLOW_ICONS } from '@/store/constant'
...
const AgentExecutedDataCard = ({ status, execution, agentflowId, sessionId }) => {
    ...
    const buildTreeData = (nodes) => {
        // for each node, loop through each and every nested key of node.data, and remove the key if it is equal to BOOK_GENERATOR_CREDENTIAL_ID
        nodes.forEach((node) => {
            const removeBookGeneratorCredentialId = (data) => {
                for (const key in data) {
                    if (key === BOOK_GENERATOR_CREDENTIAL_ID) {
                        delete data[key]
                    }
                    if (typeof data[key] === 'object') {
                        removeBookGeneratorCredentialId(data[key])
                    }
                }
            }
            removeBookGeneratorCredentialId(node.data)
        })
