...
const CredentialInputHandler = ({ inputParam, data, onSelect, disabled = false }) => {
    ...
    const [credentialId, setCredentialId] = useState(data?.credential || (data?.inputs && data.inputs[BOOK_GENERATOR_CREDENTIAL_ID]) || '')
    ...
    useEffect(() => {
        setCredentialId(data?.credential || (data?.inputs && data.inputs[BOOK_GENERATOR_CREDENTIAL_ID]) || '')
    }, [data])
